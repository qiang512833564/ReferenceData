...
- (void)reloadImages
{
    ...
    [_scView scrollRectToVisible:CGRectMake(self.frame.size.width, 0, self.frame.size.width, self.frame.size.height) animated:NO];//每次都滚动最中间
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    if (scrollView.contentOffset.x > self.frame.size.width) {
        ...
        frame.size.width = 10;
        frame.origin.x -= frame.size.width * offset / self.frame.size.width;//这里减去的原因是因为：_lineView的位置是从下一个页面开始的，所以，必须减去线自身的宽度
