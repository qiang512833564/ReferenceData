...
#pragma mark - 根据图片来设置imageFrame（在self.view上的位置）
-(void)setShowViewFrame
{
    ...
    viewShow.backgroundColor = [UIColor purpleColor];
...
-(void)markAfterFaceDetect:(NSArray *)features
{
    ...
    for (CIFaceFeature *f in features)
    {
        ...
        aRect.origin.y = self.viewShow.bounds.size.height - aRect.size.height - aRect.origin.y;//self.bounds.size（貌似脸部获得的bounds坐标是与显示在模拟器上的y坐标相反，因此要这样计算，而不是直接取bounds）
...
#pragma mark ------ 根据imageView.frame的（width与height最小的一边来设置）
