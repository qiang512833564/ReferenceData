...
- (void)upDate{
    ...
    [tempImageArray addObject:[self.pics lastObject]];//+1
    
    for (id obj in self.pics) {
        [tempImageArray addObject:obj];//+count
    }
    [tempImageArray addObject:[self.pics objectAtIndex:0]];//+1
    ...
    for (id obj in self.pics) {
        ...
        if (self.titleColor) {
            title.backgroundColor = self.titleColor;
        }else
        {
        ...
        [title setAlpha:0.7];
        NSString *string = [obj objectForKey:@"title"];
        if(self.width == 0)
        {
            self.width = string.length;
        }
        else if(self.width > string.length)
        {
            self.width = string.length;
        }
        NSString *sub = [string substringToIndex:self.width];
        if(sub.length != string.length)
        {
            sub = [NSString stringWithFormat:@"  %@...",sub];
        }
        [title setText:[NSString stringWithFormat:@"%@", sub]];
        
        [title setTextColor:[UIColor colorWithRed:0.95 green:0.91 blue:0.85 alpha:1.0]];
    ...
    if ([self.pics count]>3) {
        scrollTimer = [NSTimer scheduledTimerWithTimeInterval:10.0f target:self selector:@selector(scrollTopic) userInfo:nil repeats:YES];
    }
