...
//    NSLog(@"download segment start, fileName = %@,url = %@",self.fileName,self.downloadUrlString);
    ...
    ASIHTTPRequest *asiRequest = [ASIHTTPRequest requestWithURL:[NSURL URLWithString:[self.downloadUrlString stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]]];//将downloadUrlString字符串编码，使其中的汉字变成二进制
//    NSLog(@"------%@------",asiRequest.url);
    /*
     stringByReplacingPercentEscapesUsingEncoding:(NSStringEncoding)encoding解码，将二进制还原成汉字或者其他
     */
    [asiRequest setTemporaryFileDownloadPath: self.tmpFileName];
    /*
     下载过程中，数据将被保存在一个临时文件中。这个文件路径保存于temporaryFileDownloadPath。当请求成功完成，下面的事情之一会发生：
