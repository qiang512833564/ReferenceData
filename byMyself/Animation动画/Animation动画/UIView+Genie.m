                          ...
                          completion:(void (^)())completion
{
    ...
    [slices enumerateObjectsUsingBlock:^(CALayer *layer, NSUInteger idx, BOOL *stop) {
        CAKeyframeAnimation *anim = [CAKeyframeAnimation animationWithKeyPath:@"transform"];
        anim.duration = duration;
#pragma mark ----这里是动画的关键。。。。（整个思路是：先将屏幕截图成需要的图片，再讲图片分隔成若干部分，然后，在把每个部分计算出需要的transform组，然后在这里进行动画，，其实边界的弧度：也是通过计算transform来获得的）
...
- (CATransform3D) transformRect: (CGRect) rect toTrapezoid: (BCTrapezoid) trapezoid
{
    ...
    CATransform3D transform = {a/i, d, M_PI_2, g/i, b/i, e/i, 0, h/i, 0, 0, 1, 1, c/i, f/i, 0, 1.0};
    /*
