...
int main (int argc, const char * argv[])
{
    ...
    FILE *stream ;
    if((stream=fopen("/Users/lizhongqiang/Desktop/头像1.jpg","w+"))==NULL)
    {
                ...
                if (client_fds[i]!=0) {
                    ...
                    if (FD_ISSET(client_fds[i], &server_fd_set)) {
                        ...
                        if (byte_num > 0) {
                            ...
                            recv_msg[byte_num] = '\0';
                            printf("客户端(%d):%s-----%d-----%d\n",i,recv_msg,sizeof(recv_msg),strlen(recv_msg));
                            
                            [data appendBytes:recv_msg length:sizeof(recv_msg)];
                            
                            char msg[] = "file content";
                            //strlen(char*）函数求的是字符串的实际长度，它求得方法是从开始到遇到第一个'\0'，如果你只定义没有给它赋初值，这个结果是不定的，它会从aa首地址一直找下去，直到遇到'\0'停止。
                            //fseek(stream, 0, SEEK_END);
                            fwrite(recv_msg, sizeof(recv_msg), 1, stream);
                            
                           // printf("adaadadadadad");
                        }else if(byte_num < 0){
                            ...
                            //
                            fclose(stream);
                            [data writeToFile:@"/Users/lizhongqiang/Desktop/头像2.jpg" atomically:YES];
                            printf("客户端(%d)退出了\n",i);
                        }
                    }
                }
