...
//  }
  ...
  /**
   *因为：
   1. 调用 require('UIView') 后，就可以直接使用 UIView 这个变量去调用相应的类方法了，require 做的事很简单，就是在 JS 全局作用域上创建一个同名变量，变量指向一个对象，对象属性__isCls 表明这是一个 Class，__clsName 保存类名，在调用方法时会用到这两个属性。
    ...
    2. 在JSEngine.m里面，已经通过正则，把script里所有的方法，都变成了__C,
   * 为Object对象增加“__c”属性，就是为了让require创建的对象，都会调用该__C方法,
    ...
    for (var methodName in methods) {
      ...
      (function(){                                     //此闭包为了保持执行环境中的methodName的值
       var originMethod = methods[methodName]
        console.log(methods instanceof Object, arguments instanceof Array,methodName,originMethod,methods.class)//通过打印，可以看出
        newMethods[methodName] = [originMethod.length, function() {     //originMethod.length 表示function的参数个数(JS的function对象定义的属性 length 声明了函数期望的参数个)
          ...
          try {
            global.self = args[0]
            args.splice(0,1)//splice() 方法向/从数组中添加/删除项目，然后返回被删除的项目。
                            //参数一，表示添加或者删除元素的位置
