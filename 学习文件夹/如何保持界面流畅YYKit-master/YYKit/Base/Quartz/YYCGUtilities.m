...
CGContextRef YYCGContextCreateGrayBitmapContext(CGSize size, CGFloat scale) {
    size_t width = ceil(size.width * scale);//ceil向上取整
    ...
    /*
     ...
     height bitmap的宽度，单位为像素
     bitsPerComponent  内存中像素的每个组件的位数。例如，对于32位像素格式和RGB颜色空间，你应该将这个值设置为8
     bytesPerRow       bitmap的每一行在内存中所占的比特数
     colorspace        bitmap上下文使用的颜色空间
     bitmapInfo        指定bitmap是否包含alpha通道，像素中alpha通道的相对位置，像素组件是整形还是浮点型等信息的字符串。
     */
    CGContextRef context = CGBitmapContextCreate(NULL, width, height, 8, 0, space, kCGBitmapByteOrderDefault | alphaInfo);//这里需要的width、height需要的是pixels像素，因而是ceil(size.width * scale)
    CGColorSpaceRelease(space);
    if (context) {
        CGContextTranslateCTM(context, 0, height);
        CGContextScaleCTM(context, scale, -scale);
