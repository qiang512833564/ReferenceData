...
#pragma mark --- 这里对signalBlock函数返回的RACSigal进行了错误信号捕获，使得外面对该信号进行错误信号订阅无效
	...
	RACMulticastConnection *errorsConnection = [[[newActiveExecutionSignals
		flattenMap:^(RACSignal *signal) {
			return [[signal
				ignoreValues]//忽略所有的信号值
				catch:^(NSError *error) {//捕获错误信号
					return [RACSignal return:error];//是通过对[RACReturnSignal return:value];的调用
				}];
		}]
		deliverOn:RACScheduler.mainThreadScheduler]
		publish];
