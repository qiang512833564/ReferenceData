...
- (M80AttributedLabelURL *)urlForPoint: (CGPoint)point
{
    ...
    //CGRectInset(self.bounds, 0, -kVMargin)在高度上放大矩形--距离为5---正值代表缩小
    //这里的意思是：判断点击事件是否在该Label的bounds以内
    ...
    CTFrameGetLineOrigins(_textFrame, CFRangeMake(0,0), origins);//可以得到每行的起始位置
    /*第二个参数：
     ...
     如果CFRangeMake的第二个值为0，则表示从index开始的位置，到最后一行的起始点结束----也就是需要遍历每行起始点的范围
     */
    
    CGAffineTransform transform = [self transformForCoreText];//这个是为了使几何坐标转换成手机屏幕坐标
    CGFloat verticalOffset = 0; //不像Nimbus一样设置文字的对齐方式，都统一是TOP,那么offset就为0
    
    for (int i = 0; i < count; i++)
    {
        NSLog(@"line-------------%ld",count);
        CGPoint linePoint = origins[i];//获取每行的起点
        
        CTLineRef line = CFArrayGetValueAtIndex(lines, i);//获取行单元
        CGRect flippedRect = [self getLineBounds:line point:linePoint];//得到每行的绘制范围
        CGRect rect = CGRectApplyAffineTransform(flippedRect, transform);//把flippedRect转换成手机坐标系的rect
        
        rect = CGRectInset(rect, 0, -kVMargin);
        rect = CGRectOffset(rect, 0, verticalOffset);
        
        if (CGRectContainsPoint(rect, point))//判断手势点击的点，是否在每行单元的范围内
        {
            CGPoint relativePoint = CGPointMake(point.x-CGRectGetMinX(rect),
                                                point.y-CGRectGetMinY(rect));
            CFIndex idx = CTLineGetStringIndexForPosition(line, relativePoint);//根据相对于行单元起点位置的点击时间点，来确定区域内点处文字范围
            //返回值是一个string的index，
...
- (CGAffineTransform)transformForCoreText
{
    ...
    /*
     效果相当于
     CGContextTranslateCTM(ctx,o,rect.size.height);
...
- (CGRect)getLineBounds:(CTLineRef)line point:(CGPoint) point
{
    ...
    CGFloat width = (CGFloat)CTLineGetTypographicBounds(line, &ascent, &descent, &leading);//得到每行的范围
...
- (M80AttributedLabelURL *)linkAtIndex:(CFIndex)index
{
    for (M80AttributedLabelURL *url in _linkLocations)
    {
        NSLog(@"%@-----%@",url,_linkLocations);
        if (NSLocationInRange(index, url.range))//判断index是否在url.range的范围
        {
            return url;
        }
    }
            ...
            lineOrigin:(CGPoint)lineOrigin
{
    ...
    for (CFIndex k = 0; k < runCount; k++)
    {
        ...
        NSRange intersectedRunRange = NSIntersectionRange(lineRunRange, range);//判断单个字符单元是否在改行的范围内
...
- (void)appendAttachment: (M80AttributedLabelAttachment *)attachment
{
     ...
     */
    callbacks.getAscent     = ascentCallback;//(上行高度)从原点到字体中最高（这里的高深都是以基线为参照线的）的字形的顶部的距离，ascent是一个正值
    callbacks.getDescent    = descentCallback;//(下行高度)从原点到字体中最深的字形底部的距离，descent是一个负值
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGAffineTransform transform = CGAffineTransformScale(CGAffineTransformMakeTranslation(0, self.bounds.size.height), 1.f, -1.f);//[self transformForCoreText];
    CGContextConcatCTM(ctx, transform);//把当前的图形矩阵与后面参数transform关联起来----在这里实现的作用就是：翻转图形上下文，因为这里默认的是以左下角为原点
    
    [self recomputeLinksIfNeeded];//自动检测是否含有比较明显的链接
    
    NSAttributedString *drawString = [self attributedStringForDraw];//获得附文本
    if (drawString)
    {
        [self prepareTextFrame:drawString rect:rect];//根据drawString来设置CTFramesetterRef，进而生产出_textFrame
        [self drawHighlightWithRect:rect];//当触摸链接的时候，绘制链接处的点击高亮颜色
...
- (void)drawHighlightWithRect: (CGRect)rect
{
    if (self.touchedLink && self.highlightColor)
    {
        ...
        NSInteger numberOfLines = [self numberOfDisplayedLines];//根据用户设置和计算来--获取行数
        ...
        for (CFIndex i = 0; i < numberOfLines; i++)
        {
            ...
            NSRange intersectedRange = NSIntersectionRange(lineRange, linkRange);//获取两个range的交叉范围
            //返回值如果是0，则代表无交叉
            ...
            CGRect highlightRect = [self rectForRange:linkRange
                                               inLine:line
                                           lineOrigin:lineOrigins[i]];//根据range来得到rect
...
- (void)recomputeLinksIfNeeded
{
    ...
    [self computeLink:text
                 sync:sync];//当检测的长度，大于设置值得时候，会异步处理(检测的内容一般是整个_attributedString的内容)
}
...
////自动检测是否含有比较明显的链接
{
    ...
    typedef void (^LinkBlock) (NSArray *);//定义一个block型变量
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    if (self.touchedLink == nil)
    {
        ...
        self.touchedLink =  [self urlForPoint:point];//获取可以点击的链接
    }
